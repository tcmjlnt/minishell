...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/14 19:07:49 by tjacquel          #+#    #+#             */
/*   Updated: 2025/06/14 20:35:47 by tjacquel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strndup_noquotes(char *src, size_t n)
{
	...
	if (!dest)
	{
		printf ("ft_strndup_Noquotes malloc failure\n");
		return(NULL);
	}
	i = 0;
	j = 0;
...
{
	size_t	len = 0;
	size_t	len_noquotes = 0;
	char	*res = NULL;
	if (ac == 2)
	{
		len = ft_strlen(av[1]);
		len_noquotes = ft_strnlen_noquotes(av[1], len);
		res = ft_strndup_noquotes(av[1], len);
		printf("av[1]: %s\nlen: %zd\nlen_noquotes: %zd\nres: %s\n", av[1], len, len_noquotes, res);
