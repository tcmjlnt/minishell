...
/*   Updated: 2025/06/24 19:48:08 by tjacquel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*fill_env_tab(t_env *entry)
{
	...
	line = gc_mem(GC_ALLOC, len, NULL, GC_ENV); // pk le gc_type c'est GC_CMD ici et pas GC_ENV ??
	if (!line)
...
char	**env_to_env_tab_for_execve(t_env *env)
{
	...
	env_tab = gc_mem(GC_ALLOC, sizeof(char *) * (count + 1), NULL, GC_ENV);  // pk le gc_type c'est GC_CMD ici et pas GC_ENV ??
	if (!env_tab)
